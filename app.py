...
# Inisialisasi session state - SANGAT PENTING untuk diletakkan di awal
...
# Variabel untuk kode verifikasi
verification_code = ""
...
async def run_client():
    try:
        ...
        async def code_callback():
            ...
            st.info("Masukkan kode verifikasi Telegram di kolom yang disediakan.")
        ...
        logger.error(error_msg)
        if 'log_messages' in st.session_state and 'running' in st.session_state:
            st.session_state.log_messages.append({
                'time': datetime.now().strftime("%H:%M:%S"),
                'message': error_msg,
                'error': True
...
if st.session_state.running:
    verification_input = st.text_input("Masukkan Kode Verifikasi dari Telegram (jika diminta):", key="code_input")
    if verification_input:
        verification_code = verification_input
